...
class WikiChangeMonitor:
    def __init__(
        ...
        *,
        ...
        watch_category_redirects: bool = True,
        ...
        self.watch_category_redirects = watch_category_redirects
        ...
        async with self._lock:
            ...
            if self.detect_redirects:
                redirect_ids = list(dict.fromkeys(
                    ...
                    and (self._category_redirect_candidate(change) or
                         any((umo, change.rcid) not in pending_keys
                             for umo in self._matching_targets(change, previous_members, update_titles=False)))
                    for revid in (change.old_revid, change.revid) if revid > 0))
                if redirect_ids:
                    try:
            ...
            # Inspect the exact first revision even when the alias has no own
            # categories. Only its direct target is matched; never follow a
            # current redirect chain to reinterpret historical source.
            indirect: dict[int, dict[str, list[str]]] = {}
            uncertain_redirects: set[int] = set()
            candidates = [change for change in changes if self._category_redirect_candidate(change)]
            target_titles = {}
            for change in candidates:
                if change.revid not in redirect_targets:
                    uncertain_redirects.add(change.rcid)
                    continue
                target = redirect_targets[change.revid].partition('#')[0].strip().removeprefix(':')
                if target:
                    target_titles[change.rcid] = target
            target_page_ids = {}
            if target_titles:
                try:
                    target_page_ids = await self.client.page_ids_for_titles(list(dict.fromkeys(target_titles.values())))
                except Exception:
                    uncertain_redirects.update(target_titles)
                    LOGGER.warning('Redirect target lookup unavailable; retrying recent new pages within grace window')
            for change in candidates:
                page_id = target_page_ids.get(target_titles.get(change.rcid, ''), 0)
                if page_id > 0:
                    indirect[change.rcid] = self._matching_redirect_categories(change, page_id, previous_members)
            consumed: list[RecentChange] = []
            deferred_timestamps: list[str] = []

            for change in changes:
                if change.rcid in uncertain_redirects and self._should_defer_new_page(change):
                    deferred_timestamps.append(change.timestamp)
                    continue
                targets = self._matching_targets(change, previous_members)
                direct_umos = set(targets)
                for umo, scopes in indirect.get(change.rcid, {}).items():
                    merged = targets.setdefault(umo, [])
                    merged.extend(scope for scope in scopes if scope not in merged)
                ...
                for umo, scopes in new_targets:
                    recipient_lines = diff_lines
                    if umo not in direct_umos and umo in indirect.get(change.rcid, {}):
                        recipient_lines = [*diff_lines, DiffLine('redirect', '📌按重定向目标匹配订阅分类')]
    ...
    def _category_redirect_candidate(self, change: RecentChange) -> bool:
        return (self.detect_redirects and self.watch_category_redirects
                and change.change_type == 'new' and change.revid > 0
                and any(sub.kind == SUBSCRIPTION_CATEGORY and
                        (not sub.start_timestamp or change.timestamp >= sub.start_timestamp)
                        for sub in self.state.subscriptions))

    def _matching_redirect_categories(
        self, change: RecentChange, target_pageid: int,
        previous_members: dict[tuple[str, str, str], tuple[set[int], set[str]]],
    ) -> dict[str, list[str]]:
        result: dict[str, list[str]] = {}
        for sub in self.state.subscriptions:
            if sub.kind != SUBSCRIPTION_CATEGORY or (sub.start_timestamp and change.timestamp < sub.start_timestamp):
                continue
            previous_ids, _ = previous_members.get(sub.key, (set(), set()))
            if target_pageid not in previous_ids and target_pageid not in sub.member_page_ids:
                continue
            scopes = result.setdefault(sub.umo, [])
            if sub.label not in scopes:
                scopes.append(sub.label)
        return result
