...
DEFAULT_USER_AGENT = "AstrBot-MediaWiki/1.8.0"
MAX_TITLES = 5
...
class MediaWikiClient:
    ...
    async def page_ids_for_titles(self, titles: list[str]) -> dict[str, int]:
        """Resolve direct local titles, not redirect chains, to current page IDs.

        Keys retain input spelling. A missing/invalid/interwiki title maps to 0.
        API normalization and variant conversion are honored; no page extracts
        or revision content are requested.
        """
        result: dict[str, int] = {}
        ...
        for offset in range(0, len(unique), 50):
            ...
            params = self._base_query_params()
            params.update(prop='info', titles='\x1f' + '\x1f'.join(batch), converttitles='1')
            data = await self._request(params)
            query = data.get('query')
            if not isinstance(query, dict):
                raise MediaWikiHTTPError('目标页面查询缺少 query 字段')
            mappings = {}
            for key in ('normalized', 'converted'):
                for mapping in _as_list(query.get(key)):
                    mappings[str(mapping.get('from', ''))] = str(mapping.get('to', ''))
            pages = {str(page.get('title', '')): int(page.get('pageid', 0) or 0)
                     for page in _as_list(query.get('pages'))
                     if not page.get('missing') and not page.get('invalid')}
            for title in batch:
                canonical = title
                visited = set()
                while canonical in mappings and canonical not in visited:
                    visited.add(canonical)
                    canonical = mappings[canonical]
                result[title] = max(0, pages.get(canonical, 0))
        return result
