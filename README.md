...
当前版本：`1.8.0`。支持监控“指向分类成员的新建重定向”，别名自身无需加入分类；新安装默认每条最多 700 字符。

### 1.8.0 分类条目的新别名通知

新增 `change_watch_category_redirects=true`，与 `change_detect_redirects=true` 同时开启时生效：

- 已订阅分类（含配置深度内的子分类）中有条目 A，新建别名 B 的首修订为 `#REDIRECT [[A]]` 时，即使 B 没有任何分类，也会通知对应会话。`A#章节` 按 A 匹配，通知中保留章节。
- 从新建记录的精确修订 ID 读取源码，按站点指令别名确认重定向。再分批查询直接目标的基本信息，仅规范化标题及转换字形，不请求摘要、不跟随当前重定向链。目标必须是有效本地页面，按 pageid 匹配分类树的当前/刷新前成员快照，不声称还原历史时刻的分类归属。
- 同一会话通过别名自身分类和目标分类同时命中时合为一条，分类名称去重；只通过目标命中时标注“📌按重定向目标匹配订阅分类”。沿用移动附带重定向的严格关联去重及发送失败队列。别名不被写入分类成员快照。
- 本功能只扩展**新建重定向页面**的分类匹配，不扫描全站已有重定向，不扩展单条目订阅，也不自动监控无分类的既有别名后续编辑、目标变化或取消。后者若通过原有页面/分类订阅命中，仍使用 1.7.0 的状态通知。
- 仅在存在时间范围内有效的分类订阅时扫描新建页。新增页的历史源码每批最多 20 个修订；不同直接目标去重后，每批最多 50 个标题查询基本信息。不使用最近更改中的当前重定向标记代替历史源码判断。
- 源码不可读或目标查询失败时，近期新页面在 `change_new_page_grace_seconds` 窗口内保留时间水位重试，不提前发送部分命中的通知；超过窗口按已确认的直接订阅处理或略过。将窗口设为 0 则不等待。不存在、无效、跨 Wiki 目标或无法确认的源码不会被猜测为命中。
...
+30 N | 编辑者 | 12:00
订阅：LoveLive!
https://zh.moegirl.org.cn/index.php?curid=123
↪新建重定向 → ｢目标条目#章节｣
📌按重定向目标匹配订阅分类
```

升级不会覆盖已有配置；如果先前关闭了 `change_detect_redirects`，需要同时开启它。状态格式仍为 v4。本次未包含上一轮评估中提出的全文缓存、发送退避等独立优化，暂缓的模板专用语义识别也未加入。
